...
public class ObjetosGraficos {
    ...
    final int W_LABEL = 20; // Ancho etiqueta
    final int H_LABEL = 20; // Alto etiqueta

    final int OFFSET = 5;   // Nº de pixels que avanza en cada movimiento
    
    final int DEMORA = 5; // Milisegundos que esperamos para realizar el siguiente movimiento

    final int PROB_CAMBIADIRECCION = 5; // Porcentaje por el que se cambia de dirección

    final static int N_HILOS = 100;  // Nº de hilos, y etiquetas que se mostrarán
    ...
    public boolean colisionaConOtras( JLabel label,  Rectangle newPos) {//metodo devuelve true si colisiona y false si no
        synchronized (hilos) {
            for (Hilo hilo : hilos) {
                if (hilo != null) {
                    JLabel otroLabel = hilo.getLabel();
                    if (otroLabel != label) {
                        if (newPos.intersects(otroLabel.getBounds())) {
                            return true;
                        }
                    }
                }
    ...
    public Point getPosLibre() {
        synchronized (hilos) {
            ...
            int x,y;
            do {
                x = this.rnd.nextInt((int) rPanel.getWidth() - 30);
                y = this.rnd.nextInt((int) rPanel.getHeight() - 30);
                rect.setLocation(x, y);
            } while (colisionaConOtras(null, rect));//recorre hasta que la posición esté libre, entoces la devuelve
    ...
    class Hilo extends Thread {
        ...
        private void desplazaEtiqueta() {
            ...
            if (!colisionaConOtras(label, rLabel)) {
                this.label.setBounds(rLabel);
            }
        }
